...
You qualified for a Blue Belt 🥋🔵 and got a final team grade of 5.31/6 for M3. Excellent work! You're demonstrating advanced skills.
...
| **Functionality**                        | 5.1 out of 6 | 20%       | [See Details](#functionality)              |
...
| **QA (Testing)**                         | 5.44 out of 6           | 20%        | [See Details](#qa-testing)                 |
| **Documentation**                        | 4.3 out of 6 | 10%       | [See Details](#documentation)              |
| **Autonomy**                             | 6 out of 6     | 5%         | [See Details](#autonomy)                   |
| **Final Team Grade**                     | **5.31 out of 6** |            |                                            |
...
- When switching to offline mode, the app got really slow and the user profile was sometimes replaced with default values. There might be really high timeouts that are bit long to wait for and make this offline mode not extremely pleasant to use.

- Most of your features are well implemented! There were a few minor issues related to performance, but they are in general working well!
...
While we tested your app, we encountered the following bugs and UX flaws:

- The friends count in the profile does not reflect our actual number of friends.

- The friend page is extremly slow to load, there might be unoptimized queries or logic.

- When searching for a friend, if no match are found, the bar just disappears.

- Delete group does work, but it stays on the screen. If we then click on "leave the group", it makes the app crash.

- The online status is not persistent in your profile -> you can set it to offline, but going back to your profile to edit it, it will stay green.

- Some UI pages had text formatting issues with the phone form factor we used.
...
Here is a list of UX observations that we made when using your app:

- Cancel button to the right is not really intuitive (UX design).

- Leaderboard button is cropped due to the phone screen size.

- Participants in the events could be linked with accounts. (In general, a full page with the event's details could have been a good addition)

- The friend card is not clickable, only the user's name is, which makes it harder to navigate to the user's profile. 

- Group title can have multiple lines, which is not ideal, because it can take all the screen space.

- The backstack is not handled correctly when adding/editing groups.

- The badge screen is blinking when loading (looks like the screen is being loaded twice).

- An onboarding feature to briefly explain how your app is used could have been good. (-> What can be done on the app, how, ...)
...
- While the project is exceptionally well-structured, MainActivity.kt file remains slightly crowded by housing the entire NavHost definition alongside the activity's lifecycle logic.

- You still have 79 issues in Sonar. If they are not relevant, you should mark them as such, otherwise some important informations might get lost in it.
...
- Your code coverage is quite good, at more than 91% and you wrote generally good tests!

- The CI you used had some issues with the way you wrote the tests, making it hard for you to do your work, but you spent a lot of time on it and did your best to make it work. 

For this part, you received 5.44/6.
...
- Your README is quite minimalistic and incomplete.

- The figma and the architecture diagram are good !

- Your project code is well-documented, except for some private functions and some testing files.
...
- The meetings went really well and you all had professional and respectful attitudes.
...
Based on the above points, your grade for this M3 milestone is 5.31/6. If you are interested in how this fits into the bigger grading scheme, please see [project README](https://github.com/swent-epfl/public/blob/main/project/README.md) and the [course README](https://github.com/swent-epfl/public/blob/main/README.md).
