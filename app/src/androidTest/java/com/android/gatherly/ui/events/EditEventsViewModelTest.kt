...
class EditEventsViewModelTest {
    ...
    fun canFindProfiles1() {
        runTest {
            ...
            editEventsViewModel.updateParticipant(participantString)
            assert(participantString == editEventsViewModel.uiState.participant) { "\'Profile\' should update the search string" }
    ...
    fun canSaveEvent1() {
        runTest {
            ...
            //wait
            assert(eventsRepository.getEvent(event.id).title == modifiedEvent.title) { "The event is not modified" }
        }
    }
    ...
    fun canSaveEvent2() {
        runTest {
            ...
            editEventsViewModel.saveEvent()
            //wait
            assert(eventsRepository.getEvent(event.id).creatorName == modifiedEvent.creatorName) { "The event is not modified" }
        }
    }
    ...
    fun canDeleteEvent() {
        runTest {
            ...
            //wait
            assert(editEventsViewModel.uiState.backToOverview) {"Successfully deleting should go back to overview events"}
        }
    }

    // This function fills the profile repository with the created profiles, and the event repository with the created event
