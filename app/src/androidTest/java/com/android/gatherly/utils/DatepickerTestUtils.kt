...
/**
 * Navigates to and selects a specific date in a Material3 DatePicker dialog within a Compose UI
 * test.
 *
 * **Constraint:** This function strictly supports **English (`en`)** and **French (`fr`)** locales.
 * It will throw an exception if run in any other locale configuration.
 *
 * The function automates the date picker interaction by:
 * 1. Validating the test environment locale.
 * 2. Opening the year selector via the header and selecting the [targetYear].
 * 3. Identifying the currently displayed month from the header text.
 * 4. Navigating forward or backward through months until the [targetMonth] is reached.
 * 5. Locating and clicking the [targetDay] to finalize the selection.
 *
 * @param targetYear The year to navigate to (e.g., 2024).
 * @param targetMonth The target month as an **English** month name string (e.g., "JANUARY",
 *   "December"). Note: This must match [java.time.Month] enum names, regardless of the active
 *   locale.
 * @param targetDay The day of the month to select (e.g., 15).
 * @throws IllegalStateException If the [Locale.getDefault] language is not "en" or "fr".
 * @see [ComposeTestRule]
 * @see [java.time.Month]
 */
fun ComposeTestRule.navigateInDatePicker(targetYear: Int, targetMonth: String, targetDay: Int) {
  ...
  // Fail if the locale is not supported
  if (locale.language != "en" && locale.language != "fr") {
    throw IllegalStateException(
        "Test running in unsupported locale: '${locale.language}'. navigateInDatePicker only supports FR and EN.")
  }
...
/** Check if a node has a LiveRegion field. */
