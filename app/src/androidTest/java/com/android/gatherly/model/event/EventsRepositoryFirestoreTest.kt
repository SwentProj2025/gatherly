...
class EventsRepositoryFirestoreTest : FirestoreEventsGatherlyTest() {
  ...
  @Test
  fun getEvent_returns_exact_event() = runTest (timeout = 120.seconds) {
    repository.addEvent(event1)
  ...
  @Test
  fun getEvent_throws_when_not_found() = runTest (timeout = 120.seconds) {
    try {
  ...
  @Test
  fun addEvent_with_location_stores_correctly() = runTest (timeout = 120.seconds) {
  ...
  @Test
  fun addEvent_stores_all_timestamps_correctly() = runTest (timeout = 120.seconds) {
  ...
  @Test
  fun editEvent_updates_existing_event() = runTest (timeout = 120.seconds) {
    repository.addEvent(event1)
  ...
  @Test
  fun editEvent_throws_security_exception_when_not_creator() = runTest (timeout = 120.seconds) {
    // Add event as user1
    repository.addEvent(event1)
  ...
  @Test
  fun deleteEvent_removes_it() = runTest (timeout = 120.seconds) {
    repository.addEvent(event1)
    repository.addEvent(event2)
  ...
  @Ignore("Fails due to new Firestore event rules — to be fixed in a separate PR.")
  @Test
  fun deleteEvent_throws_security_exception_when_not_creator() = runTest (timeout = 120.seconds) {
    // Add event as user1
    repository.addEvent(event1)
  ...
  @Test
  fun addParticipant_adds_user_to_list() = runTest (timeout = 120.seconds) {
    repository.addEvent(event1)
  ...
  @Test
  fun addParticipant_multiple_users() = runTest (timeout = 120.seconds) {
    repository.addEvent(event1)
  ...
  @Test
  fun removeParticipant_removes_user_from_list() = runTest (timeout = 120.seconds) {
