...
fun MapScreen(
  ...
  LaunchedEffect(Unit) { viewModel.initialiseCameraPosition(context) }
  ...
  Scaffold(
      ...
      content = { pd ->
        ...
        GoogleMap(
            ...
            onMapClick = { _ -> viewModel.clearSelection() }) {
              ...
              sheetState = sheetState, onDismissRequest = { viewModel.clearSelection() }) {
                EventSheet(
                    ...
                    onGoToEvent = {
                      ...
                      viewModel.onItemConsulted(selectedEvent.id)
          ...
          ModalBottomSheet(
              sheetState = sheetState, onDismissRequest = { viewModel.clearSelection() }) {
                ToDoSheet(
                    ...
                    onGoToToDo = {
                      // Track consulted item before navigation
                      viewModel.onItemConsulted(selectedToDo.uid)
                      viewModel.clearSelection()
                      goToToDo()
                    },
                    onClose = { viewModel.clearSelection() })
