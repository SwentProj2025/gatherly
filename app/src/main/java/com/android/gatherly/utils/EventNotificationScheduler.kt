...
class EventNotificationScheduler(private val notificationsRepository: NotificationsRepository) {
  ...
  suspend fun sendEventReminderNotification(userId: String, eventId: String) {
    val existing =
        notificationsRepository.getUserNotifications(userId).any {
          it.type == NotificationType.EVENT_REMINDER && it.relatedEntityId == eventId
        }
